...
def parseArgs():
    ...
    parser = argparse.ArgumentParser(
        description="Enumerate valid usernames based on the requests response times."
    )
    parser.add_argument(
        "-u", "--username", default=None, required=True, help="Username"
    )
    parser.add_argument(
        "-t",
        "--threads",
    ...
    parser.add_argument(
        ...
        default=20,
        required=False,
        help="Number of login tries (default: 20)",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    group.add_argument(
        "-S", "--show", default=None, action="store_true", help="Show graph"
    )
...
if __name__ == "__main__":
    ...
    random_username = "".join(
        [random.choice(string.ascii_letters) for k in range(len(options.username))]
    )
    ...
    average_rand_user = sum(statistics[random_username]) / len(
    ...
    average_real_user = sum(statistics[options.username]) / len(
    ...
    if diff < 0:
        print(
            "[+] A login with a real username was %f ms slower than a random username on an average of %d tries."
            % (abs(diff), options.samples)
        )
    else:
        print(
            "[+] A login with a random username was %f ms faster than a real username on an average of %d tries."
            % (abs(diff), options.samples)
        )
    ...
    if options.show is not None or options.file is not None:
        ...
        plt.fill_between(
            [x for x in range(len(statistics[random_username]))],
            [average_rand_user for x in range(len(statistics[random_username]))],
            step="pre",
            alpha=0.25,
            color="red",
        )
        ...
        plt.fill_between(
            [x for x in range(len(statistics[options.username]))],
            [average_real_user for x in range(len(statistics[options.username]))],
            step="pre",
            alpha=0.25,
            color="blue",
        )
